...
export class Equals {
	...
	#setMode(player, mode, all, force) {
		if (all) {
			for (let k of this.DATA.PLAYERS) {
				if (this.DATA.MODES[k] !== "spectate" || force === true) {
	...
	nextPlayer(player) {
		...
		console.log("Current mode", this.DATA.MODES[player])
		...
		if (this.DATA.MODES[player] !== "spectate") {
			this.#setMode(player, "out", true)
		}
		...
		switch (action) {
			...
			case "gold":
				...
				if (this.DATA.PLAYERS.length > 2) {
					...
					this.#setMode(player, "end", true, true)
				}
				break
			case "pass":
				// pass on turn
			...
			case "final":
				...
				this.#setMode(player, "score", true, true)
				break
		}
